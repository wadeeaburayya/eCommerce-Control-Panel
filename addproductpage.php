 ...
 $filename = $_FILES["uploadfile"]["name"];
    $tempname = $_FILES["uploadfile"]["tmp_name"];    
      ...
      if(empty(trim($_POST["productName"]))){
        $productName_err="fill";}
      if(empty(trim($_POST["productPrice"]))){
        $productPrice_err="fill";}
      if(empty(trim($_POST["productImage"]))){
        $productImage_err="fill";}
        if(empty($productName_err) && empty($productPrice_err) && empty($productImage_err)){
        $sql = "INSERT INTO productcategory (productCategoryID, productName, productPrice, productImage) VALUES ('$productCategoryID', '$productName', '$productPrice', '$folder')";
        if($stmt = mysqli_prepare($con, $sql)){
        $param_productCategoryID=$productCategoryID;
        $param_productName=$productName;
        $param_productPrice=$productPrice;
    
        mysqli_stmt_bind_param($stmt, "isis", $param_productCategoryID, $param_productName, $param_productPrice, $folder);
...
<body style="background-color: #c5c5c5">
    ...
    <nav class="w3-sidebar w3-bar-block w3-small w3-hide-small w3-center">
        ...
        <a href="addproductpage.php" class="w3-bar-item w3-button w3-padding-large w3-black">
    ...
    <div class="w3-top w3-hide-large w3-hide-medium" id="myNavbar">
        ...
        <i onclick="location.href='addproductpage.php';" style="cursor: pointer; font-size:36px; color: green;" class='fas fa-folder-plus' ></i>
    </div>
    ...
    <div class="login12 login123">
                ...
                <label>Price</label>
                <input type="text" id="email" name="productPrice">
                ...
                <option disabled selected>-- Select Category --</option>
                    <?php
                     ...
                     $records = mysqli_query($con, "SELECT productCategoryID From productcategory");  // Use select query here 

                         while($data = mysqli_fetch_array($records))
                         {
                             echo "<option value='". $data['productCategoryID'] ."'>" .$data['productCategoryID'] ."</option>";  // displaying data in option menu
                        }	
                        ?>  
                 </select>

                <label for="img">Image</label>
                <input type="file" id="img" name="productImage" accept="image/*">
